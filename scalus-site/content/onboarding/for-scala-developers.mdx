---
title: Scalus for Scala Developers
sidebarTitle: For Scala Developers
---

import { Steps, Callout } from 'nextra/components'

# Scalus Onboarding for Scala Developers

Welcome! You know Scala, the JVM ecosystem, and functional programming. Now learn how to build smart contracts for the Cardano blockchain using the language you already love.
...
### Phase 2: Cardano Fundamentals (1-2 hours)
...
1. **UTxO (Unspent Transaction Output)** - Like a "check" holding value
   ```scala
   // Think of it as:
   case class UTxO(
     address: Address,      // Who can spend it
     value: Value,          // How much ADA/tokens
     datum: Option[Data]    // Custom data attached
   )
   ```

2. **Validator** - A function that says "yes/no" to spending
   ```scala
   @Compile
   object MyValidator extends Validator:
     def spend(datum: Data, redeemer: Data, ctx: ScriptContext): Unit =
       // If this completes without throwing, spending is allowed
       // If it throws, transaction is rejected
       require(someCondition, "Nope!")
   ```

3. **Script Context** - Transaction info passed to your validator
   ```scala
   case class ScriptContext(
     txInfo: TxInfo,        // Transaction details
     redeemer: Redeemer,    // Data provided by spender
     scriptInfo: ScriptInfo // What's being validated
   )
   ```
...
### Phase 3: Production Development (2-3 hours)

**Goal:** Build tested, optimized validators ready for mainnet
...
**🎯 Quick Win:** Deploy a fully tested validator to Cardano testnet with execution cost analysis.
</Steps>
---
...
- [ ] **[Real-World Examples](https://github.com/nau/scalus/tree/master/scalus-examples)** - Study production patterns
...
object Good extends Validator:
  def spend(...) =
    val data = scalus.prelude.List(1, 2, 3)  // ✅ Correct!
```
...
### For Experienced Scala Devs (Total: ~2-3 hours)

1. **[Check Hello Cardano Smart contract](/docs/get-started#get-your-first-scalus-validator)** → 5 min
2. **[First Smart Contract](/docs/smart-contract/developing-smart-contracts)** → 30 min
3. **[eUTxO Model Explainer](https://docs.cardano.org/about-cardano/learn/eutxo-explainer)** → 15 min
4. **[The EUTxO-Model - Plutus Pioneer Program](https://www.youtube.com/watch?v=ulYDNaEKf4g)** - 25 min
5. **[Validators in Depth](/docs/smart-contract/validators)** → 20 min
6. **[Supported Features](/docs/language-guide/support)** → 10 min
7. **[Testing Smart Contracts](/docs/smart-contract/testing)** → 20 min
8. **Build your own validator** → 45 min

### For Scala Beginners (Total: ~1 day)

Complete all Phase 1-3 checklists + study the [Examples Repository](https://github.com/nau/scalus/tree/master/scalus-examples) to see patterns in action.
...
### vs. Writing Plutus in Haskell

**Haskell Plutus:**
```haskell
{-# INLINABLE validator #-}
validator :: Datum -> Redeemer -> ScriptContext -> Bool
validator d r ctx = traceIfFalse "error" $ checkCondition d r ctx
```
...
object Validator extends Validator:
  def spend(datum: Data, redeemer: Data, ctx: ScriptContext): Unit =
    require(checkCondition(datum, redeemer, ctx), "error")
```

**Advantages:**
- Familiar Scala syntax
- Better IDE support (IntelliJ IDEA, Metals)
- Integrated testing with ScalaTest
- Share code between on-chain and off-chain
...
validator {
  fn spend(datum: Datum, redeemer: Redeemer, ctx: ScriptContext) -> Bool {
    expect Some(pkh) = datum.owner
    ctx.transaction.extra_signatories |> list.has(pkh)
  }
}
```
...
object Validator extends Validator:
  def spend(datum: Data, redeemer: Data, ctx: ScriptContext): Unit =
    val pkh = datum.to[PubKeyHash]
    require(ctx.txInfo.signatories.contains(pkh))
```

**Advantages:**
- Type-safe serialization with `FromData` derivation
- Full Scala ecosystem for off-chain code
...
### ✅ Completed Phase 1?
Great! You've seen Scalus in action. Move to Phase 2 to understand Cardano's eUTxO model.

### ✅ Completed Phase 2?
You now understand blockchain fundamentals! Move to Phase 3 and build production validators.

### ✅ Completed Phase 3?
You're production-ready! Explore:
- **[Advanced Optimizations](/docs/smart-contract/optimisations)** - Reduce execution costs
- **Transaction Builder(In progress)** - Build Cardano transactions
- **[Examples Repository](https://github.com/nau/scalus/tree/master/scalus-examples)** - Advanced patterns
