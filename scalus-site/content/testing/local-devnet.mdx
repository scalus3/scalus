---
title: Local Cardano Devnet for Integration Testing
sidebarTitle: Local Devnet
description: Run Cardano smart contract integration tests locally with a Docker-based devnet. Submit transactions, validate Plutus scripts, and test with a real Cardano node — no testnet dependencies.
---
...
The `TestContext` includes a `BlockfrostProvider` connected to the container's Yaci Store API. This provides a Blockfrost-compatible interface locally — your [transaction building](/docs/transactions) code works the same against local devnet and real networks:
