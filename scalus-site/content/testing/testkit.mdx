...
In practice, contracts are tested by multiple **agents** (participants) doing different things — a donor donating, a recipient withdrawing, an attacker trying to steal funds. The agent model makes this explicit:

1. **Contract under test** — the smart contract validator
2. **Agents** — participants that interact with the contract, each generates actions based on state
3. **Test configuration** = state extraction + set of agents + slot delays

Each agent is a `ContractTestAgent[S]` that, given the current blockchain state, returns the actions it can take:
...
```scala
// Simple agent — builds one transaction (or none if it can't act)
val bidder = ContractTestAgent.simple[AuctionState](
    "bidder-alice",
    (reader, state) => buildBidTx(reader, state).map(Some(_)).recover { case _ => None }
)

// Multi-action agent — builds multiple transactions
val donorGroup = ContractTestAgent.multi[CampaignState](
    "donor-group",
    (reader, state) => buildDonateTxs(reader, state)
)

// Agent with attack variations — base tx + standard attack patterns
val attacker = ContractTestAgent.withVariations[AuctionState](
    "attacker",
...
### Composing Agents into a Step

Use `ContractStepVariations.fromAgents` to combine agents into a single step:

```scala
val step = ContractStepVariations.fromAgents[AuctionState](
    extract = (reader, ec) => extractAuctionState(reader)(using ec),
    agents = Seq(
        HonestBidderAgent(alice),
        HonestBidderAgent(bob),
        attackerAgent   // attack txs should be rejected by the contract
    ),
...
The key pattern: mix honest agents with attacker agents. The contract should reject all attack transactions while accepting honest ones. If an attacker's transaction gets accepted, the invariant check fails and ScalaCheck reports the minimal failing sequence.

### Two Paths

- **Simple (single interaction type):** Implement `ContractStepVariations` directly with `makeBaseTx` + `variations`
- **Multi-agent:** Define agents, compose with `ContractStepVariations.fromAgents`
...
object ContractTestAgent {
    // Agent with base tx + attack variations
    def withVariations[S](agentName: String, baseTx: ..., txVariations: TxVariations[S] = TxVariations.empty): ContractTestAgent[S]

    // Agent from a single optional transaction
    def simple[S](agentName: String, buildTx: (BlockchainReader, S) => Future[Option[Transaction]]): ContractTestAgent[S]

    // Agent from multiple transactions
    def multi[S](agentName: String, buildTxs: (BlockchainReader, S) => Future[Seq[Transaction]]): ContractTestAgent[S]
}
```

### ContractStepVariations.fromAgents
...
Combines all agents' actions with slot delays into a single `ContractStepVariations`. The resulting step overrides `allActions` directly — `makeBaseTx`/`variations` are not used.
