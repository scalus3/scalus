...
Scalus provides full support for Plutus V3 primitive types, allowing you to write smart contracts using familiar Scala syntax while targeting the Cardano blockchain.
...
// Operations
val concat = ByteString.fromHex("dead") ++ ByteString.fromHex("beef")
...
// Comparison
val eq = fromHex == ByteString.fromHex("deadbeef")
val compare = fromHex < ByteString.fromHex("ffffff")
```

### String

Strings are UTF-8 encoded text values.

```scala
val greeting = "Hello, Cardano!"
val concat = "Hello" ++ " " ++ "World"
val eq = "test" == "test"
val byteString = ByteString.fromString("convert to bytes")
```
...
// Comparison
val dataEq = intData == Builtins.iData(BigInt(42))
```
