...
Functions are first-class values in Scalus, enabling functional programming patterns essential for smart contract development. Scalus supports both named functions (`def`) and anonymous functions (lambdas).
...
**NOT SUPPORTED** - Scalus doesn't support default parameter values:
...
**Workaround:** Use function overloading alternatives or explicit parameters:
...
compile {
  ...
  // Or use Option for optional parameters
  def greetOpt(name: String, greeting: Option[String]): String =
    val greetingText = greeting.getOrElse("Hello")
    s"$greetingText, $name"

  greetOpt("Alice", Some("Hi"))  // "Hi, Alice"
  greetOpt("Bob", None)          // "Hello, Bob"
}
```

## Named Arguments

**NOT SUPPORTED** - Scalus doesn't support named arguments:
...
**Workaround:** Use positional arguments or case classes:

```scala
case class TransferParams(from: Account, to: Account, amount: BigInt)
...
**NOT SUPPORTED** - Scalus doesn't support variable argument lists:
...
**Workaround:** Use Lists explicitly:

```scala
compile {
  def sum(numbers: List[BigInt]): BigInt =
    numbers.foldLeft(BigInt(0))(_ + _)
...
compile {
  ...
  def addWith10And20(c: BigInt): BigInt =
    add3(BigInt(10), BigInt(20), c)

  addWith10And20(BigInt(30))  // 60
  ...
  addWith10(BigInt(20), BigInt(30))  // 60
}
```
...
compile {
  ...
  doubleAndIncrement(BigInt(5))  // 11
