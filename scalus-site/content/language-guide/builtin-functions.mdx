...
Do **not** use `Builtins.multiIndexArray`. It is not a real Plutus builtin — no released Plutus
version (or Aiken) implements it, and Scalus serializes it with an invented flat tag, so any script
using it fails to decode/evaluate under real Plutus tooling. Access elements individually with
`indexArray` instead. It will be enabled once upstream Plutus defines the builtin.
</Callout>
