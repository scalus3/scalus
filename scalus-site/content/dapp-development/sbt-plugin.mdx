...
come with this plugin already wired in — along with `profile`, `emulator`, and `devkit` commands.
If you're starting from a template, the `blueprint` and `deploy` tasks below work out of the box; the
setup below is for adding the plugin to an existing project.
