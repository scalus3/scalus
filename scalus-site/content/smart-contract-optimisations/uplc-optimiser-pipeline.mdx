---
title: UPLC Optimiser Pipeline in Scalus
sidebarTitle: UPLC Optimiser Pipeline
description: UPLC optimiser passes in Scalus — EtaReduce, Inliner, beta-reduction, constant folding, StrictIf, ForcedBuiltinsExtractor, CaseConstrApply. Reduce script execution costs.
---
...
Scalus optimizes Untyped Plutus Core (UPLC) scripts to reduce script size and execution costs. `toUplcOptimized()` and `Options.default` enable optimization automatically. `Options.default` also retains error traces; use `Options.release` to remove them for release measurements. Trace retention can affect script size and which expressions can be folded.
