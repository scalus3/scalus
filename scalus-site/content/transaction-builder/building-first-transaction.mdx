...
### JVM-Only Blocking Completion

On the JVM, there's a blocking overload that takes a `Provider` directly and returns `TxBuilder` synchronously:
...
val transaction = TxBuilder(cardanoInfo)
  .payTo(recipientAddress, Value.ada(10))
  .complete(provider, sponsorAddress, timeout = 30.seconds)  // Blocking (JVM only)
  .sign(signer)
  .transaction
```

You can also use the `await` extension to block on any `Future`:
...
val transaction = TxBuilder(cardanoInfo)
  .payTo(recipientAddress, Value.ada(10))
  .complete(asyncProvider, sponsorAddress)
  .await(30.seconds)  // Block with timeout (JVM only)
  .sign(signer)
  .transaction
```
