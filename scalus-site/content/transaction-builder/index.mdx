...
TxBuilder is designed to work together with the rest of the Scalus ecosystem.
...
For cleaner code, you can use `txBuilder` with a context `CardanoInfo`:
...
val tx = txBuilder                           // Uses the given CardanoInfo
  .spend(utxo)
...
<Callout type="info">
  TxBuilder is purely functional - each method returns a new instance with updated state, and it's fully immutable.
  Only `build()` can throw while evaluating scripts and balancing the transaction.
  The `complete()` method returns a `Future` that may fail with script evaluation or balancing errors.
</Callout>
...
TxBuilder(env, evaluator)
  ...
  .build(changeTo = changeAddress)  // Scripts are evaluated here
```
