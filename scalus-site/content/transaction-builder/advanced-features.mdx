...
### NodeEmulator for Testing

The `NodeEmulator` supports transaction chaining naturally by tracking UTXOs across submissions:

```scala copy
val emulator = NodeEmulator(initialUtxos)
