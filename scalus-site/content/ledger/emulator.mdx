...
The `Emulator` is a local implementation of the [`Provider`](/ledger/provider) trait that operates entirely in-memory, without any network connection. Unlike `BlockfrostProvider` which interacts with a real blockchain, the `Emulator` keeps the entire UTxO set in memory and validates transactions locally.

When you submit transactions to the Emulator, it runs an extensive suite of [ledger rules](/ledger/ledger-rules) to validate them, but doesn't maintain consensus since there's no actual chain underneath—only an emulator of a single node.

This makes the Emulator ideal for unit tests and rapid development, where launching a local node would be inconvenient and network delays would slow down the development cycle.
...
- [Provider](/ledger/provider) - Understanding the Provider interface
- [Ledger Rules](/ledger/ledger-rules) - Transaction validation rules
- [Protocol Parameters](/ledger/protocol-parameters) - Network configuration
