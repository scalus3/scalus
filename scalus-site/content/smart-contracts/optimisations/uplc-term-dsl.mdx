...
description: Build hand-crafted UPLC validators using Scalus Term DSL -- maximum control over the generated code, similar to Plutarch in Haskell.
---
...
Here's the same PreimageValidator from the [Optimizing Scala Code](/docs/smart-contracts/optimisations/scala-optimizations) chapter, built entirely with the Term DSL. This targets PlutusV4 and uses `Case` on List and `Case` on Bool for maximum efficiency:
