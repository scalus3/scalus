...
class CardanoLedgerConformanceTest extends AnyFunSuite {
    ...
    private def validateVectorWithContextCapture(
      ...
      (String, Boolean, CardanoMutator.Result, Option[Seq[(Redeemer, ScriptContext, ScriptHash)]])
    ] =
                ...
                if captureContexts then
                    try
                        val evaluator = PlutusScriptEvaluator(
                          ...
                          mode =
                              EvaluatorMode.EvaluateAndComputeCost // Use counting mode to avoid budget errors
                        )
                        Some(evaluator.evalPlutusScriptsWithContexts(transaction, state.utxos))
                    catch
                        case e: Exception =>
                            println(
                              s"Failed to capture ScriptContext: ${e.getClass.getSimpleName}: ${e.getMessage}"
                            )
                            None
                else None
    ...
    /** Log budget differences between original transaction redeemers and Scalus computed budgets.
      *
      * Uses PlutusScriptEvaluator to get the computed budgets, then compares them with the original
      * redeemer budgets from the transaction.
      */
    private def logBudgetDifferences(
        ...
        val originalRedeemers =
            transaction.witnessSet.redeemers.map(_.value.toMap).getOrElse(Map.empty)
        ...
        try {
            ...
            val evaluatedRedeemers =
                evaluator.evalPlutusScriptsWithContexts(transaction, state.utxos)

            for (evaluatedRedeemer, _, _) <- evaluatedRedeemers do
                val origExUnitsOpt =
                    originalRedeemers.get((evaluatedRedeemer.tag, evaluatedRedeemer.index))
    ...
    test("Conformance ledger rules test") {
        ...
        val groups = failed
            .groupBy {
                case (_, _, _, Left(ex)) =>
                    ex.getClass.getSimpleName + ": " + ex.getMessage.take(80)
    ...
    /** Test that logs budget differences for all conformance test vectors.
      *
      * This test validates transactions while logging budget differences between the original
      * transaction redeemers (from cardano-ledger test vectors) and the budgets computed by Scalus.
      *
      ...
      *
      * These differences are due to test vectors being generated with an older version of
      * cardano-ledger. Scalus matches current Haskell `uplc` exactly.
      */
    test("Conformance test with budget difference logging") {
        ...
        for vectorName <- vectorPatterns do
            try {
                validateVectorWithContextCapture(
                  vectorName,
                  captureContexts = false,
                  logBudgetDiff = true
                )
            } catch {
                case e: Exception =>
    ...
    test("Verify argument application costs are correct") {
        ...
        val vm = PlutusVM.makePlutusV1VM()
        ...
        result match
            case Result.Success(_, budget, costs, _) =>
                ...
                println(
                  s"Difference: memory=${budget.memory - expectedMemory}, steps=${budget.steps - expectedCpu}"
                )

                // Print breakdown
                def sumCosts(
                    costs: collection.Map[ExBudgetCategory, collection.Seq[ExUnits]]
                ): Map[ExBudgetCategory, ExUnits] =
                    costs.view
                        .mapValues(
                          _.foldLeft(ExUnits(0, 0))((acc, e) =>
                              ExUnits(acc.memory + e.memory, acc.steps + e.steps)
                          )
                        )
                        .toMap
                ...
                assert(
                  budget.memory == expectedMemory,
                  s"Memory mismatch: got ${budget.memory}, expected $expectedMemory"
                )
                assert(
                  budget.steps == expectedCpu,
                  s"Steps mismatch: got ${budget.steps}, expected $expectedCpu"
                )
    ...
    test("Minimal ScriptContext encoding comparison") {
        ...
        println(
          s"Value.lovelace(1000000) CBOR: ${scalus.utils.Hex.bytesToHex(lovelaceData.toCbor)}"
        )
    }
    ...
    test("Compare overhead across multiple test vectors") {
        ...
        do
            ...
            if plutusScriptsMap.nonEmpty then
                ...
                val datumsMapping =
                    transaction.witnessSet.plutusData.value.toMap.view.mapValues(_.value).toSeq
                ...
                if originalRedeemers.nonEmpty then
                    ...
                    val redeemer =
                        Redeemer(redeemerKey._1, redeemerKey._2, redeemerData, origExUnits)

                    val purpose =
                        LedgerToPlutusTranslation.getScriptPurposeV1(transaction, redeemer)
                    ...
                    vm.evaluateScriptDebug(applied) match
                        case Result.Success(_, budget, _, _) =>
                            ...
                            println(
                              f"$name%-8s | mem=${origExUnits.memory}%8d cpu=${origExUnits.steps}%10d | mem=${budget.memory}%8d cpu=${budget.steps}%10d | $memDiff%+7d | $cpuDiff%+9d"
                            )
        ...
        if count > 0 then
            println("-" * 80)
            println(
              f"Average overhead: memory=${totalMemDiff / count}%+d, cpu=${totalCpuDiff / count}%+d"
            )
    }
        ...
        for case (path, vector) <- loadAllVectors(vectorName) do
            ...
            if name == testCase then
                ...
                if plutusScriptsMap.nonEmpty then
                    ...
                    println(
                      s"Redeemer data memory usage: ${MemoryUsage.memoryUsageData(redeemerData)}"
                    )
                    println(s"ScriptContext memory usage: ${MemoryUsage.memoryUsageData(ctxData)}")
                    datum.foreach(d =>
                        println(s"Datum memory usage: ${MemoryUsage.memoryUsageData(d)}")
                    )
                    ...
                    baseResult match
                        case Result.Success(_, budget, _, _) =>
                            println(s"Base budget (no args): $budget")
                        case Result.Failure(ex, budget, costs, _) =>
                            ...
                            sumCosts2(costs).toSeq
                                .filter(_._1.toString.startsWith("Step"))
                                .sortBy(-_._2.steps)
                                .foreach { case (cat, cost) =>
                                    println(
                                      f"  $cat%-30s: mem=${cost.memory}%6d, cpu=${cost.steps}%8d"
                                    )
                                }
                    ...
                    def countTerms(term: Term): Map[String, Int] = {
                        ...
                        def go(t: Term): Unit = t match {
                            ...
                            case Term.Case(scrutinee, cases) =>
                                counts("Case") += 1; go(scrutinee); cases.foreach(go)
                        }
                        go(term)
                        counts.toMap
                    }
                    ...
                    val argApplicationCost =
                        args.size * (machineParams.machineCosts.constCost.memory + machineParams.machineCosts.applyCost.memory)
                    val argApplicationCostCpu =
                        args.size * (machineParams.machineCosts.constCost.steps + machineParams.machineCosts.applyCost.steps)
                    println(
                      s"\nExpected argument application cost: ${args.size} args * (constCost + applyCost)"
                    )
                    println(
                      s"  = ${args.size} * (${machineParams.machineCosts.constCost} + ${machineParams.machineCosts.applyCost})"
                    )
                    println(s"  = mem=${argApplicationCost}, cpu=${argApplicationCostCpu}")
                    println(s"Actual overhead: mem=1302, cpu=336382")
                    println(
                      s"Unexplained: mem=${1302 - argApplicationCost}, cpu=${336382 - argApplicationCostCpu}"
                    )
                    ...
                    println(
                      s"To evaluate with Haskell: uplc evaluate --input ${appliedFlatFile} --input-format flat --counting"
                    )
                    ...
                    println(
                      s"ScriptContext CBOR hex (first 200): ${scalus.utils.Hex.bytesToHex(ctxCbor).take(200)}..."
                    )
                    ...
                    println(
                      f"  inputs:      mem=${MemoryUsage.memoryUsageData(inputsData)}%6d, cbor=${inputsData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  outputs:     mem=${MemoryUsage.memoryUsageData(outputsData)}%6d, cbor=${outputsData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  fee:         mem=${MemoryUsage.memoryUsageData(feeData)}%6d, cbor=${feeData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  mint:        mem=${MemoryUsage.memoryUsageData(mintData)}%6d, cbor=${mintData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  dcert:       mem=${MemoryUsage.memoryUsageData(dcertData)}%6d, cbor=${dcertData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  withdrawals: mem=${MemoryUsage.memoryUsageData(withdrawalsData)}%6d, cbor=${withdrawalsData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  validRange:  mem=${MemoryUsage.memoryUsageData(validRangeData)}%6d, cbor=${validRangeData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  signatories: mem=${MemoryUsage.memoryUsageData(signatoriesData)}%6d, cbor=${signatoriesData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  data:        mem=${MemoryUsage.memoryUsageData(dataFieldData)}%6d, cbor=${dataFieldData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  id:          mem=${MemoryUsage.memoryUsageData(idData)}%6d, cbor=${idData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  purpose:     mem=${MemoryUsage.memoryUsageData(purposeData)}%6d, cbor=${purposeData.toCbor.length}%5d bytes"
                    )
                    println(
                      f"  TOTAL ctx:   mem=${MemoryUsage.memoryUsageData(ctxData)}%6d, cbor=${ctxData.toCbor.length}%5d bytes"
                    )
                    ...
                    println(
                      s"Redeemer CBOR hex: ${scalus.utils.Hex.bytesToHex(redeemerData.toCbor)}"
                    )
                    ...
                    result match
                        case Result.Success(term, budget, costs, logs) =>
                            ...
                            summedCosts.toSeq.sortBy(-_._2.steps).foreach { case (cat, cost) =>
                                println(
                                  f"  $cat%-50s: mem=${cost.memory}%8d, cpu=${cost.steps}%10d"
                                )
                            }
                        case Result.Failure(ex, budget, costs, logs) =>
                            println(
                              s"Evaluation failed: ${ex.getClass.getSimpleName}: ${ex.getMessage}"
                            )
    ...
    /** Analyze budget differences across multiple failing test vectors to find patterns.
      ...
      *   - CPU: +336382
      *
      * This proves that Scalus computes execution budgets correctly and matches Haskell's current
      * `uplc` tool exactly. The difference comes from the test vectors being generated with a
      * different (older) version of cardano-ledger that used different ScriptContext encoding or
            ...
            for case (path, vector) <- loadAllVectors(pattern) do
                if vector.success then
                    try {
                        ...
                        val transaction = Transaction.fromCbor(Hex.hexToBytes(vector.cbor))
                        val state =
                            LedgerState.fromCbor(Hex.hexToBytes(vector.oldLedgerState)).ruleState
                        ...
                        for (redeemerKey, (redeemerData, origExUnits)) <- originalRedeemers do
                            ...
                            val plutusScriptsMap =
                                AllWitnessesScripts.allWitnessesPlutusScriptsMap(transaction)
                            if plutusScriptsMap.nonEmpty then
                                val (_, plutusScript) = plutusScriptsMap.head
                                val scriptBytes = plutusScript match
                                    case Script.PlutusV1(bs) => bs
                                ...
                                val datumsMapping =
                                    transaction.witnessSet.plutusData.value.toMap.view
                                        .mapValues(_.value)
                                        .toSeq
                                val txInfoV1 = LedgerToPlutusTranslation.getTxInfoV1(
                                  transaction,
                                ...
                                val purpose = LedgerToPlutusTranslation.getScriptPurposeV1(
                                  transaction,
                                  redeemer
                                )
                                val scriptContext =
                                    scalus.ledger.api.v1.ScriptContext(txInfoV1, purpose)
                                val ctxData = scriptContext.toData
                                ...
                                val datum: Option[Data] = {
                                    ...
                                    if inputs.isDefinedAt(redeemer.index) then
                                        ...
                                        state.utxos
                                            .get(inputRef)
                                            .flatMap(_.datumOption match
                                                case Some(DatumOption.Hash(hash)) =>
                                                    datumsMapping.toMap.get(hash)
                                                case Some(DatumOption.Inline(data)) => Some(data)
                                                case None                           => None)
                                    else None
                                }
                                ...
                                val uplcOutput =
                                    s"uplc evaluate --input ${appliedFlatFile} --input-format flat --counting".!!
                                Files.delete(appliedFlatFile)

                                val cpuPattern = "CPU budget:\\s+(\\d+)".r
                                val memPattern = "Memory budget:\\s+(\\d+)".r
                                val uplcCpu = cpuPattern
                                    .findFirstMatchIn(uplcOutput)
                                    .map(_.group(1).toLong)
                                    .getOrElse(0L)
                                val uplcMem = memPattern
                                    .findFirstMatchIn(uplcOutput)
                                    .map(_.group(1).toLong)
                                    .getOrElse(0L)
    ...
    test("Compare Scalus vs Haskell uplc on identical applied program") {
        ...
        for case (path, vector) <- loadAllVectors(vectorName) do
            ...
            if name == testCase then
                ...
                import scala.sys.process.*
                val uplcOutput =
                    s"uplc evaluate --input /tmp/scalus_applied.flat --input-format flat --counting".!!
                ...
                val memPattern = "Memory budget:\\s+(\\d+)".r
                val uplcCpu =
                    cpuPattern.findFirstMatchIn(uplcOutput).map(_.group(1).toLong).getOrElse(0L)
                val uplcMem =
                    memPattern.findFirstMatchIn(uplcOutput).map(_.group(1).toLong).getOrElse(0L)

                println(s"\n=== COMPARISON ===")
                println(
                  s"Test vector (cardano-ledger):  mem=${origExUnits.memory}, cpu=${origExUnits.steps}"
                )
                println(
                  s"Scalus:                        mem=${scalusBudget.memory}, cpu=${scalusBudget.steps}"
                )
                ...
                println(s"\nDifferences:")
                println(
                  s"  Scalus vs Test Vector:  mem=${scalusVsVector._1}, cpu=${scalusVsVector._2}"
                )
                ...
                assert(
                  scalusBudget.memory == uplcMem,
                  s"Memory mismatch: Scalus=${scalusBudget.memory}, uplc=$uplcMem"
                )
                assert(
                  scalusBudget.steps == uplcCpu,
                  s"CPU mismatch: Scalus=${scalusBudget.steps}, uplc=$uplcCpu"
                )
