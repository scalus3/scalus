...
class PricebetValidatorTest extends AnyFunSuite, ScalusTest {
    ...
    test("Player joins successfully") {
        ...
        assert(
          ...
          joinResult.budget == (ExUnits(memory = 128662, steps = 41_698556))
        )
    }
    ...
    test("Player wins with oracle rate above threshold") {
        ...
        assert(
          winResult.budget == (ExUnits(memory = 108348, steps = 33_270921))
        )
    }
    ...
    test("Owner times out after deadline") {
        ...
        assert(
          timeoutResult.budget == (ExUnits(memory = 43739, steps = 14_700074))
        )
    }
    ...
    test("Oracle updates successfully") {
        ...
        assert(
          updateResult.budget == (ExUnits(memory = 65589, steps = 24_706128))
        )
    }
