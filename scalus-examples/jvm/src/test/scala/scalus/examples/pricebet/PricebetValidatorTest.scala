...
class PricebetValidatorTest extends AnyFunSuite, ScalusTest {
    ...
    test("Player joins successfully") {
        ...
        assert(
          ...
          joinResult.budget == (ExUnits(memory = 123462, steps = 40_866556))
        )
    }
    ...
    test("Player wins with oracle rate above threshold") {
        ...
        assert(
          winResult.budget == (ExUnits(memory = 105648, steps = 32_838921))
        )
    }
    ...
    test("Owner times out after deadline") {
        ...
        assert(
          timeoutResult.budget == (ExUnits(memory = 42539, steps = 14_508074))
        )
    }
    ...
    test("Oracle updates successfully") {
        ...
        assert(
          updateResult.budget == (ExUnits(memory = 63389, steps = 24_354128))
        )
    }
