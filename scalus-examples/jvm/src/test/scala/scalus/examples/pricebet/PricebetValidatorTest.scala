...
class PricebetValidatorTest extends AnyFunSuite, ScalusTest {
    ...
    test("Player joins successfully") {
        ...
        assert(
          ...
          joinResult.budget == (ExUnits(memory = 59825, steps = 27_549_301))
        )
    }
    ...
    test("Player wins with oracle rate above threshold") {
        ...
        assert(
          winResult.budget == (ExUnits(memory = 64339, steps = 24_296_257))
        )
    }
    ...
    test("Owner times out after deadline") {
        ...
        assert(
          timeoutResult.budget == (ExUnits(memory = 31147, steps = 11_946_458))
        )
    }
    ...
    test("Oracle updates successfully") {
        ...
        assert(
          updateResult.budget == (ExUnits(memory = 51005, steps = 21_342_961))
        )
    }
