...
/** Represents different JIT compilation strategies for UPLC terms.
  *
  * This enum provides a unified interface for different JIT implementation approaches, each with
  * different performance characteristics and trade-offs:
  *
  *   - **Mincont**: Minimal continuation-based approach using heap-allocated frames. Stack-safe
  *     with moderate performance overhead due to eval loop.
  *   - **NativeStack**: Direct JVM stack-based approach with no continuation overhead. Maximum
  *     performance but not stack-safe (can overflow on deep recursion).
  *   - **Hybrid**: Intelligent switching between NativeStack (for shallow recursion) and Mincont
  *     (for deep recursion), providing best-of-both-worlds performance.
  *
  * @example
  *   {{{
  * val term: Term = ...
...
enum JITImplementation:
    /** Minimal continuation-based JIT implementation.
      *
      * Uses heap-allocated continuation frames and a trampolining eval loop to achieve stack
      * safety. Can handle arbitrary recursion depth but has moderate performance overhead (~27%
      * from eval loop).
      *
      * **Characteristics**:
    ...
    /** Native JVM stack-based JIT implementation.
      *
      * Uses direct JVM method calls with no continuation overhead. Provides maximum performance
      * through zero-indirection execution but uses JVM call stack directly, making it susceptible
      * to StackOverflowError on deep recursion.
      *
      * **Characteristics**:
    ...
    /** Hybrid JIT implementation with intelligent engine selection.
      *
      * Automatically chooses between NativeStack (for shallow recursion) and Mincont (for deep
      * recursion) based on static analysis or dynamic fallback. Provides optimal performance while
      * maintaining safety.
      *
      * @param depthThreshold
      *   Maximum recursion depth before switching to Mincont (default: 1000)
      * @param useStaticAnalysis
      *   Whether to use static depth estimation (default: true)
      * @param useDynamicFallback
      *   Whether to catch StackOverflow and retry with Mincont (default: true)
      *
      * **Characteristics**:
    ...
    /** Compiles a UPLC term using this JIT implementation strategy.
      ...
      * @return
      *   A function that evaluates the term with the given logger, budget, and params
      */
