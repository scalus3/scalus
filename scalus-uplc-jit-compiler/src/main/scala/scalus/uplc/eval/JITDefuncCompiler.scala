...
import scalus.builtin.{Builtins, ByteString, Data, BuiltinPair}
import scalus.uplc.{Constant, DefaultFun, DefaultUni, Term, NamedDeBruijn}
import scalus.uplc.eval.JITDefunc.{*, given}
import scala.annotation.switch
import scala.quoted.*
import scala.collection.mutable

/** Compiler for hybrid JIT: converts UPLC terms to defunctionalized instructions + JIT snippets. */
object JITDefuncCompiler {
  ...
  /** Compile a term to instructions, returning the entry instruction index.
    ...
    * @param env Environment mapping variable names to their stack locations
    * @return Index of the first instruction for this term
    */
