...
/** Memory usage calculation for JIT values without CekValue wrapper allocation.
  *
  * JIT represents values as native Scala types (BigInt, String, ByteString, Data, etc.)
  * instead of wrapped CekValue. This object provides memory usage calculation directly
  * on these native types, avoiding the overhead of creating VCon wrappers.
  */
object MemoryUsageJit {

    /** Calculate memory usage for JIT values directly without CekValue wrapper.
      * 
      * @param a JIT value (BigInt, String, ByteString, Data, List, Pair, etc.)
      * @return Memory usage in CostingInteger units
      */
