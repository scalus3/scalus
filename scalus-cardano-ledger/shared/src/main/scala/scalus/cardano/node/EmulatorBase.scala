...
trait EmulatorBase extends BlockchainProvider {
    ...
    private def submitWith(
        ...
        val (result, applied) = modifyState { state =>
            processTransaction(validationContext(state.context), state.ledger, transaction) match
                case Right(newLedger) =>
                    ...
                    (state.withApplied(newLedger, applied), (Right(transaction.id), Some(applied)))
                case Left(t: TransactionException) =>
                    (state, (Left(SubmitError.fromException(t)), None))
        }
        // Outside the compare-and-set, deliberately: `modifyState` re-runs its function when a
        // concurrent submission wins the race, and a listener must see one notification per
        // application rather than one per attempt. By here the state that produced `applied` is
        // the one that was committed.
        applied.foreach(notifyApplied)
        result
    }
