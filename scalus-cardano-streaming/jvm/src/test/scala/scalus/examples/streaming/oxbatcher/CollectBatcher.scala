...
/** Example: an ox-based collect-batcher.
  *
  * Direct-style equivalent of the fs2 batcher in the sibling
  * `fs2batcher` package. Same logical structure, different runtime:
  *
  *   - one `forkUser` runs the Spent subscription, removing entries
  *     from the in-memory `pending` set as our submitted sweeps are
  *     observed on chain
  *   - the main path runs the Created subscription, batched via
  *     `Flow.groupedWithin(maxCount, maxAge)`, and submits each batch
  *     as a sweep transaction
  *
  * Rollback handling is delegated to the subscription via
  * `SubscriptionOptions.noRollback = true`. Errors propagate through
  * `supervised`.
  *
  * This is a skeleton: `buildSweepTx` is `???` and there is no real
  * provider yet. It exercises the streaming API in the ox runtime to
  * mirror the fs2 example.
  */
class CollectBatcher(
    ...
    /** Start the batcher. Blocks the calling thread until either the
      * Created or Spent subscription ends (provider closed, fatal
      * error, etc.).
      */
    ...
    /** Build a sweep transaction consuming all pending UTxOs and
      * producing a single output to `config.sweepTo`. Out of scope for
      * the skeleton — wire up via `TxBuilder` when the example moves
      * to `scalus-examples`.
      */
...
/** Configuration for the ox collect batcher.
  ...
  *   via ox `Flow.groupedWithin`'s time bound)
  */
