...
This skill provides knowledge for implementing `Pretty[A]` typeclass instances in the Scalus codebase.
...
The `Pretty[A]` typeclass converts values to `org.typelevel.paiges.Doc` for flexible pretty printing.
...
- `a + b` - concatenate directly (no space)
- `a & b` - concatenate with space
- `a / b` - concatenate with line break
...
Colors cycle through 16 distinct XTerm colors: red → orange → yellow → lime → green → teal → cyan → blue → indigo → purple → magenta → pink. Colors are chosen to avoid collision with `ctr`, `lit`, and `kw` styling.
...
2. **Choose the right pattern:**
   - Case classes: show fields with `Pretty.field` or custom layout
...
given Pretty[MyCaseClass] with
    def pretty(a: MyCaseClass, style: Style): Doc =
        val fields = List(
            Pretty.field("name", text(a.name), style),
            Pretty.field("value", lit(str(a.value), style), style)
        )
...
```scala
given Pretty[MyType] = Pretty.instanceWithDetailed(
  concise = (a, style) => text(a.hash.toHex.take(8) + "..."),
  detailed = (a, style) => text(a.hash.toHex)
)
```
