...
* lambda expressions
* recursive functions, including mutually recursive top-level `def`s in `@Compile` objects
...
* forward references between local `def`s inside `compile` blocks - move mutually
  recursive functions to a top-level object annotated with `@Compile`
