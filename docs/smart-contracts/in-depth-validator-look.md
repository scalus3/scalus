...
Both branches read the transaction's validity range through `tx.validFromOrFail(msg)` and
`tx.validToOrFail(msg)`. Each returns the finite bound, or fails with the message when the range is
unbounded on that side. This matters for the `Timeout` branch: a transaction with no lower bound
has no earliest time, and a validator that substituted `0` for it would let the committer reclaim
before the timeout. The lower bound is inclusive and the upper bound is exclusive, which is why
