...
| Check | Prevents |
|-------|----------|
| `tx.validFromOrFail(...)` / `tx.validToOrFail(...)` | A transaction with no validity bound on the side the branch depends on |
...
**Never substitute a default for a missing bound.** `validFromOrFail` fails when the transaction
has no lower bound. An earlier version of this validator read the lower bound with a default of
`0` instead, which treats an unbounded transaction as happening at the Unix epoch and lets the
committer reclaim before the timeout. `validFrom` is inclusive (the earliest slot the transaction
can land in) and `validTo` is exclusive, so both comparisons use `<=`.

The validator is under 80 lines and compiles to 318 bytes of Plutus Core (pinned in `HtlcTest`);
the `HtlcContract` main below prints the size.
...
- [HtlcValidator.scala](https://github.com/nau/scalus/blob/master/scalus-examples/jvm/src/main/scala/scalus/examples/htlc/HtlcValidator.scala) – Smart contract
- [HtlcTransactions.scala](https://github.com/nau/scalus/blob/master/scalus-examples/jvm/src/main/scala/scalus/examples/htlc/HtlcTransactions.scala) – Transaction building
