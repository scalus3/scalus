...
`tx.findInputOrFail(ownRef)` scans the inputs once and fails with a message when the own input is
missing; `credential.scriptHashOrFail(msg)` projects the script hash out of the credential or fails.
Neither builds an intermediate `Option`.
...
inline override def reward(
    ...
    StakeValidator.withdraw(
      withdrawalValidator = (redeemer, validatorHash, txInfo) => {
          ...
          require(totalInputValue === expectedTotal, "Input total mismatch")
      },
      redeemer = redeemer,
...
### Callbacks return `Unit`

Every validator callback in `StakeValidator`, `TransactionLevelMinterValidator` and
`UtxoIndexer` has the type `... => Unit`. The callback performs its own `require(cond, message)`
or `fail(message)` calls; the library does not inspect a return value. A `Boolean` callback could
only ever fail with one generic message, and a `false` from a missed branch was silent.

**Migration hazard.** A lambda that returns `Boolean` still compiles against a `Unit` parameter
(Scala discards the value) and then checks nothing:

```scala
// Compiles. Checks nothing. The withdrawal amount is never verified.
withdrawalRedeemerValidator = (redeemer, lovelace) => lovelace === BigInt(0)

// Correct: the condition is inside require
withdrawalRedeemerValidator =
    (redeemer, lovelace) => require(lovelace === BigInt(0), "Withdrawal must be zero")
```

After upgrading, grep every callback passed to these patterns and wrap each condition in
`require(..., message)`.
