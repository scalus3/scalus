Source: https://scalus.org/docs/design-patterns/utxo-indexer
...
object MyValidator extends Validator {
    inline override def spend(
        ...
        UtxoIndexer.oneToOne(
          ...
          outputIdx,
          tx,
          // Each check is a require with its own message
          validator = (input, output) => {
              require(output.value.hasSameTokensAndAtLeastAda(input.resolved.value), "value")
              require(output.hasInlineDatum(expectedDatum), "datum") // your continuing datum
...
Every `UtxoIndexer` callback returns `Unit` and does its own `require`. A `Boolean` lambda compiles
against that parameter and checks nothing; see the
[migration hazard](/docs/design-patterns/withdraw-zero#callbacks-return-unit) before upgrading
existing validators.
...
UtxoIndexer.oneToMany(
  ...
  perOutputValidator = (input, idx, output) => {
      ...
      require(output.value.getLovelace >= minAmount, "Output below minimum")
  },
  collectiveValidator = (input, outputs) => {
      // Validate all outputs together
      val paid = outputs.foldLeft(BigInt(0))(_ + _.value.getLovelace)
      require(paid === input.resolved.value.getLovelace, "Outputs do not sum to the input")
  }
)
```
...
UtxoIndexer.multiOneToOneNoRedeemer(
  ...
  validator = (inIdx, input, outIdx, output) => {
      ...
      require(input.resolved.value.getLovelace === output.value.getLovelace, "Lovelace mismatch")
  }
)
```
...
UtxoIndexer.multiOneToOneWithRedeemer[MyRedeemer](
  ...
  validator = (inIdx, input, redeemer, outIdx, output) => {
      ...
      require(output.value.getLovelace >= redeemer.minPayout, "Payout below the redeemer minimum")
  }
)
```
...
**Double Satisfaction:** The singular UTxO indexer patterns (`oneToOne`, `oneToMany`) do not provide built-in protection against double satisfaction. Implement your own protection based on your contract's needs. See [Common Vulnerabilities](/docs/security/common-vulnerabilities#3-double-satisfaction).
