Source: https://scalus.org/docs/design-patterns/parameter-validation
    ...
    inline def validate(params: NFTMintParams)(scData: Data): Unit = {
        ...
        sc.scriptInfo match
            case ScriptInfo.MintingScript(policyId) =>
                // Mint exactly 1 of our token and nothing else under this policy
                require(
                  sc.txInfo.mint.hasOnly(policyId, params.tokenName, 1),
                  "Must mint exactly 1 NFT and no other token under this policy"
                )

                // Find the single output holding units of this token
...
Two details of the snippet carry security weight. `mint.hasOnly(policyId, name, 1)` rejects any
other token name under the same policy in the same transaction, which a bare
`quantityOf(...) === 1` does not. `outputs.findUniqueOrFail(p, msg)` fails when the token lands in
zero or several outputs, where `find(p).getOrFail(msg)` would silently pick the first.
...
- **[Parameterized Validators](/docs/smart-contracts/parameterized-validators)** — How to create validators with compile-time parameters
