...
Scalus itself understands `SCALUS_PROFILE=full`: every script evaluation that goes through the
ledger (Emulator, TxBuilder) or `runWithProfileReport` then writes per-script reports
(`.profile.html`/`.json`/`.csv`, plus the UPLC source map) under `target/scalus`, indexed by
`profile-manifest.json` and consumed by the
[VS Code extension](/docs/smart-contract-optimisations/vscode-extension).
See [Profiling](/docs/smart-contract-optimisations/profiling) for how to read the reports and optimise from them.
...
# target/scala-3.3.8/resource_managed/main/META-INF/scalus/blueprints/<package>/
