Source: https://scalus.org/docs/testing/protocol-version-builtins

# Plutus Builtins and Protocol Versions

The Scalus Emulator supports all Plutus builtins across all protocol versions. By default, `Emulator.withAddresses` and `Context.testMainnet()` use the latest mainnet protocol parameters -- protocol version 11 (van Rossem hard fork) -- so all builtins, including the batch6 additions (`expModInteger`, `dropList`, the array operations, `bls12_381_G1/G2_multiScalarMul`, and the `Value` builtins), are available without any extra configuration.
