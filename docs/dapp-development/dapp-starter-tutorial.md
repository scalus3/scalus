Source: https://scalus.org/docs/dapp-development/dapp-starter-tutorial
...
3. The admin must sign the transaction
  ...
  The `List` type here is `scalus.cardano.onchain.plutus.prelude.List`, not `scala.List`. On-chain code uses Scalus's own collection types.
