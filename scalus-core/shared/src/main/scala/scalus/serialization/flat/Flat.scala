...
/** Type class for the flat binary encoding (a bit-oriented serialization format, see
  * https://hackage.haskell.org/package/flat and the Plutus Core specification, Appendix D). UPLC
  * programs and SIR modules are serialized with instances of this trait.
  */
trait Flat[A]:
...
/** Flat encoding for collections as flat lists: each element is prefixed by a `1` bit, terminated
  * by a `0` bit (`List a = Nil | Cons a (List a)` with one tag bit per node).
  */
...
class EncoderState(bufferSize: Int):
    ...
    /** The bytes encoded so far, including a trailing partially-filled byte if any (unused low bits
      * are zero).
      */
...
/** Mutable bit-level cursor over a flat-encoded byte array. Mirrors [[EncoderState]]: bits are read
