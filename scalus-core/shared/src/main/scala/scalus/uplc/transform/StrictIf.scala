...
/** Optimizes if-then-else expressions by converting lazy branches to strict evaluation when safe.
  ...
  */
class StrictIf(logger: Logger = new Log()) extends Optimizer {

    /** Applies the strict if-then-else optimization.
      *
      * @param term
      ...
      *   The optimized term
      */
    def apply(term: Term): Term = strictIf(term)
    ...
    /** Performs the strict if-then-else optimization by traversing the term tree.
      ...
      *   The optimized term
      */
    private def strictIf(term: Term): Term = {
        def go(term: Term): Term = term match
            ...
            case Force(
                  Apply(
                    Apply(Apply(Force(Builtin(IfThenElse, _), _), c, _), Delay(t, _), _),
                    Delay(f, _),
                    _
                  ),
                  _
                ...
                if canBeStrict(optimizedT) && canBeStrict(optimizedF) then
                    ...
                    logger.log(
                      s"StrictIf: Converting lazy if-then-else to strict (saves 3 term evals)"
                    )
    ...
    private def canBeStrict(term: Term): Boolean = term match
        ...
        case _: Error | _: Force | _: Case | _: Constr => false
}

object StrictIf {
    def apply(term: Term): Term = new StrictIf().apply(term)
    def apply(term: Term, logger: String => Unit): Term = {
        val log = new Log()
        val result = new StrictIf(log).apply(term)
