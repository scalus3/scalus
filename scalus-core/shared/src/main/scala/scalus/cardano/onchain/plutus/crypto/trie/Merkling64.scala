...
/** Merkling primitives for radix-64 Merkle Patricia Forestry.
  *
  * Uses 6-bit path units (0-63) extracted via arithmetic, yielding 42 levels for 32-byte blake2b
  * paths (252 bits / 6 = 42, with 4 leftover bits in suffix).
  *
  * The merkle functions are unrolled (merkle2/4/8/16/32) for minimal overhead.
  */
@Compile
object Merkling64:
    import Merkling.{NullHash, NullHash2, NullHash4, NullHash8, combine}

    val NullHash16: ByteString =
        hex"209155a276ca3c2417e3876971dd587dd64ed9fcb8ef1fd6e7589ef4255c967f"
    val NullHash32: ByteString =
    ...
    /** Extracts a 6-bit value (0-63) at the given index from a 32-byte path.
      *
      * 32 bytes = 256 bits -> 42 six-bit values (252 bits) + 4 remaining bits. Every 3 bytes
      * contain 4 six-bit values (LCM(6,8) = 24 bits): Byte 0: [aaaaaa|bb] Byte 1: [bbbb|cccc] Byte
      * 2: [cc|dddddd]
      */
    ...
    // --- Unrolled merkle functions (5 levels for 32 slots) ---
    ...
    // --- Full merkle64 and sparse merkle64 (unfused, for MerklePatriciaForestry64) ---

    /** Extracts sixits (6-bit values) from a path between start and end positions. */
    def sixits(path: ByteString, start: BigInt, end: BigInt): ByteString =
        if start >= end then ByteString.empty
        else consByteString(sixit(path, start), sixits(path, addInteger(start, 1), end))

    /** Calculates Merkle root for 64 elements (6 levels of binary tree). */
    def merkle64(
        branch: BigInt,
        root: ByteString,
    ...
    /** Calculates sparse Merkle root for 64 elements (only 2 populated children). */
    def sparseMerkle64(
        me: BigInt,
        meHash: ByteString,
        neighbor: BigInt,
        ...
        if me < 32 then
            ...
            combine(
              merkle32(neighbor, neighborHash, NullHash16, NullHash8, NullHash4, NullHash2, NullHash),
