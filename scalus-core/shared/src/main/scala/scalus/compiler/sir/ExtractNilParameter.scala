...
object ExtractNilParameter {
    ...
    private val nilInjectionAnns: AnnotationsDecl =
        AnnotationsDecl(
          SIRPosition.empty,
          data = Map(
            ExtractNilApplyAnnotation -> SIR.Const(
              scalus.uplc.Constant.Bool(true),
              SIRType.Boolean,
              AnnotationsDecl(SIRPosition.empty)
            )
          )
        )
    ...
    private case class ScopeEntry(
        nilArgs: scala.List[
          (SIRType, SIR)
        ], // (nilType, argToPass) — Var(__nil) inside rhs, Constr(Nil) externally
    ...
    /** Collect nil types from a SIR tree, tracking expected types from parent nodes.
      * @param inScopeTVs
      *   TypeVars from enclosing functions' type params
      * @return
      *   set of function names that need nil (for propagation to callers)
      */
    private def collect(
        sir: SIR,
        ...
        case let @ Let(scala.List(Binding(name, _, rhs)), body, flags, _) if hasTypeParams(rhs) =>
            ...
            if nilTypes.nonEmpty || calledNilFns.nonEmpty then
                ...
                if allNilTypes.nonEmpty then needsNil.put(let, allNilTypes)
                childNames + name
            else childNames
        ...
        case IfThenElse(c, t, f, _, _) =>
            collect(c, needsNil, inScopeTVs) ++ collect(t, needsNil, inScopeTVs) ++ collect(
              f,
              needsNil,
              inScopeTVs
            )
        ...
        case Match(scr, cases, _, _) =>
            collect(scr, needsNil, inScopeTVs) ++ cases.flatMap(c =>
                collect(c.body, needsNil, inScopeTVs)
            )
    ...
    /** Walk a SIR tree tracking expected types from parent nodes. When hitting Nil, if the expected
      * type contains in-scope TypeVars, record it.
      */
    private def collectNilTypes(
        ...
        case And(a, b, _) =>
            collectNilTypes(a, None, inScopeTVs) ++ collectNilTypes(b, None, inScopeTVs)
        case Or(a, b, _) =>
            collectNilTypes(a, None, inScopeTVs) ++ collectNilTypes(b, None, inScopeTVs)
        ...
        case Constr(cname, data, args, tp, _) =>
            ...
            args.zip(argExpectedTypes.padTo(args.size, None)).flatMap { case (a, exp) =>
                collectNilTypes(a, exp.orElse(expectedType), inScopeTVs)
            }
    ...
    /** Build a map from binding name → (Let, nilTypes) for all entries in needsNil, plus a map from
      * binding name → Set[calledName] for all Let bindings with TypeParams. Then iteratively
      * propagate: if A calls B which needs nil, A also needs nil.
      */
                    ...
                    case Some(let) if !needsNil.containsKey(let) =>
                        ...
                        if calledNilNames.nonEmpty then
                            ...
                            val raw = calledNilNames.toList.flatMap { cn =>
                                nameToLet
                                    .get(cn)
                                    .flatMap(l => Option(needsNil.get(l)))
                                    .getOrElse(scala.Nil)
                            }
    ...
    /** Single top-down pass. Handles:
      ...
      *   scope entry for the immediately enclosing nil-needing function (for Nil replacement)
      * @param expectedType
      *   expected type from parent node (for determining Nil's actual type)
      */
    /** Extract TypeVar→ConcreteType substitutions from a function application. Collects from both
      * return type and parameter type vs argument type.
      */
        ...
        if subst.isEmpty then tp
            ...
            tp match
                ...
                case SIRType.CaseClass(c, args, parent) =>
                    SIRType.CaseClass(
                      c,
                      args.map(substituteType(_, subst)),
                      parent.map(substituteType(_, subst))
                    )
    ...
    private def go(
            ...
            val nilConstrs: scala.List[(SIRType, SIR)] = nilContextTypes.map { ctxTp =>
                val (constrName, dataDecl) = ctxTp match
                    case SIRType.SumCaseClass(decl, _)
                        if decl.name == SIRType.PairList.DataDeclName =>
                        (SIRType.PairList.PairNilName, decl)
                    case SIRType.SumCaseClass(decl, _) =>
                        (SIRType.List.NilConstr.name, decl)
                    case _ =>
                        (SIRType.List.NilConstr.name, SIRType.List.dataDecl)
                (ctxTp, Constr(constrName, dataDecl, scala.Nil, ctxTp, AnnotationsDecl.empty): SIR)
            }
            ...
            for entry <- needsNil.entrySet().asScala do
                ...
                if calledName != name && calledNames(rhs).contains(calledName) then
                        ...
                        nilVars.find((myTp, _) => typesMatch(myTp, calledNilTp)) match
                            ...
                            case None =>
                                (
                                  calledNilTp,
                                  nilConstrs
                                      .find((tp, _) => typesMatch(tp, calledNilTp))
                                      .map(_._2)
                                      .getOrElse(
                                        Constr(
                                          SIRType.List.NilConstr.name,
                    ...
                    val calledNewTp = calledOrigTp match
                        case SIRType.TypeLambda(params, bodyTp) =>
                            val extendedBody = forwardedNilArgs.foldRight(bodyTp) {
                                case ((tp, _), acc) => SIRType.Fun(tp, acc)
                            }
                            SIRType.TypeLambda(params, extendedBody)
                        case other =>
                            forwardedNilArgs.foldRight(other) { case ((tp, _), acc) =>
                                SIRType.Fun(tp, acc)
                            }
        ...
        case Let(bindings, body, flags, anns) =>
            ...
            Let(
              bindings.map(b =>
                  Binding(b.name, b.tp, go(b.value, needsNil, rhsScope, enclosingEntry, None))
              ),
        ...
        case And(a, b, anns) =>
            And(
              goA(a, needsNil, scope, enclosingEntry, None),
              goA(b, needsNil, scope, enclosingEntry, None),
              anns
            )
        case Or(a, b, anns) =>
            Or(
        ...
        case Constr(cname, data, args, tp, anns) =>
            val argExpectedTypes = data.constructors
                ...
                .map(_.params.map(p => Some(p.tp)))
                .getOrElse(args.map(_ => None))
            val newArgs = args.zip(argExpectedTypes.padTo(args.size, None)).map { case (a, exp) =>
                go(a, needsNil, scope, enclosingEntry, exp.orElse(expectedType), typeSubst)
            }
    ...
    /** Turn `Var(f, newTp)` into `Apply(...Apply(Var(f, newTp), __nil_0)..., __nil_n)`. Each
      * application peels one nil parameter, so the final result type = original function type.
      */
    private def injectNilApplications(
        ...
        for (ctxTp, nilArg) <- entry.nilArgs do
            ...
            current = Apply(
              current,
              concreteNilArg.asInstanceOf[AnnotatedSIR],
              resultTp,
              nilInjectionAnns
            )
            currentTp = resultTp
    ...
    private def typesMatch(a: SIRType, b: SIRType): Boolean = (a, b) match
        ...
        case (SIRType.SumCaseClass(d1, a1), SIRType.SumCaseClass(d2, a2)) =>
            d1.name == d2.name && a1.size == a2.size && a1
                .zip(a2)
                .forall((x, y) => typesMatch(x, y))
        case (SIRType.CaseClass(c1, a1, _), SIRType.CaseClass(c2, a2, _)) =>
            c1.name == c2.name && a1.size == a2.size && a1
                .zip(a2)
                .forall((x, y) => typesMatch(x, y))
