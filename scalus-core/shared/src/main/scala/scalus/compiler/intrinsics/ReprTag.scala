...
/** Lightweight mirror of `LoweredValueRepresentation` for use in `@Compile` intrinsic code.
  *
  * `LoweredValueRepresentation` cannot be used directly in `@Compile` code because its sealed
  * trait hierarchy involves types like `IndexedSeq` that the Scalus compiler plugin cannot
  * compile to SIR. This enum uses `EnumCase` flags so the plugin recognizes its cases as
  * constructors and produces `SIR.Constr` nodes. The lowering interprets these via
  * `interpretReprSIR` to reconstruct the actual `LoweredValueRepresentation`.
  */
enum ReprTag {
...
object ReprTagConvert {
    ...
    def toLoweredValueRepresentation(tag: ReprTag): LoweredValueRepresentation = tag match
        ...
        case ReprTag.PairData          => ProductCaseClassRepresentation.PairData
