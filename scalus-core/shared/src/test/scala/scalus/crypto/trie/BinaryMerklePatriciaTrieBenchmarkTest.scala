...
class BinaryMerklePatriciaTrieBenchmarkTest extends AnyFunSuite {
    ...
    private def buildTrie(n: Int): (BinaryMerklePatriciaTrie, ByteString, ByteString) = {
        val value = ByteString.fromArray(Array[Byte](1))
        val benchKey = ByteString.fromArray(Array[Byte](0, 0, 0, 0))
        var trie = BinaryMerklePatriciaTrie.empty
        var i = 0
        while i < n do
