...
class SIRLoweringRepresentationTest extends AnyFunSuite {
    ...
    test("Get default representation for List [nativeListElements=false]") {
        ...
        assert(
          generator.defaultRepresentation(listType) == SumCaseClassRepresentation.SumBuiltinList(
    ...
    test("get default representation for Option.None") {
        val optionTypeVar =
            SIRType.TypeVar("A", Some(11L), SIRType.TypeVarKind.ListAffected)
        val someTypeVar =
            SIRType.TypeVar("A", Some(12L), SIRType.TypeVarKind.ListAffected)
