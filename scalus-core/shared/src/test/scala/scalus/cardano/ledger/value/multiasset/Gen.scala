...
//    }
    ...
    def genConfigurableInnerFractional(
        ...
        } yield MultiAsset.Inner.Fractional.Fractional(poly)
    ...
    def genConfigurableInnerUnbounded(
        ...
        } yield MultiAsset.Inner.Unbounded.Unbounded(poly)

    def genConfigurableInner(
        minAssets: Int = 1,
        maxAssets: Int = 8
    ): Gen0[MultiAsset.Inner.Inner] =
        for {
            poly <- genInnerPolymorphic[Coin](minAssets, maxAssets)
        } yield MultiAsset.Inner.Inner(poly)
    ...
    private def genInnerPolymorphic[A](
        minAssets: Int = 1,
        maxAssets: Int = 8
    )(using coinArb: Arbitrary[A], m : Monoid[A] & Singleton): Gen0[CanonicalSortedMap[AssetName, A, m.type ]] = {
        import AssetName.given_Ordering_AssetName
        genMapOfSizeFromArbitrary[AssetName, A](minAssets, maxAssets)
            .map(SortedMap.from[ledger.AssetName, A](_))
            .map(CanonicalSortedMap.fromSortedMap)
    }
