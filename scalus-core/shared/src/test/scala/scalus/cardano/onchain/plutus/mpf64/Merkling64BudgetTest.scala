...
/** Microbenchmark comparing extraction and merkle costs across radix-16 and radix-64.
  *
...
class Merkling64BudgetTest extends AnyFunSuite {
    ...
    test("extraction cost: nibble vs sixit") {
        ...
        val header =
            f"${"Position"}%-12s | ${"nibble cpu"}%12s | ${"sixit cpu"}%12s | ${"sixit/nibble"}%13s"
        info(header)
        info("-" * header.length)
        ...
        for (label, nibbleIdx, sixitIdx) <- positions do
            ...
            val ratio =
                if nb.steps > 0 then f"${sx.steps.toDouble / nb.steps}%13.3f" else "         N/A"
            info(
              f"${label}%-12s | ${nb.steps}%12d | ${sx.steps}%12d | $ratio"
            )
    ...
    test("merkle cost: merkle16 vs merkle64") {
        ...
        val header =
            f"${"Branch"}%-10s | ${"m16 mem"}%8s | ${"m16 cpu"}%12s | ${"m64 mem"}%8s | ${"m64 cpu"}%12s | ${"m64/m16"}%8s"
        info(header)
        info("-" * header.length)
        ...
        for i <- branches16.indices do
            ...
            info(
              f"${b16}/${b64}%-10s | ${m16.memory}%8d | ${m16.steps}%12d | ${m64.memory}%8d | ${m64.steps}%12d | ${m64.steps.toDouble / m16.steps}%8.3f"
            )
    ...
    test("sparseMerkle cost: sparse16 vs sparse64") {
        ...
        val header =
            f"${"Positions"}%-16s | ${"s16 mem"}%8s | ${"s16 cpu"}%12s | ${"s64 mem"}%8s | ${"s64 cpu"}%12s | ${"s64/s16"}%8s"
        info(header)
        info("-" * header.length)
        ...
        for i <- cases16.indices do
            ...
            info(
              f"${me16},${nb16}/${me64},${nb64}%-16s | ${s16.memory}%8d | ${s16.steps}%12d | ${s64.memory}%8d | ${s64.steps}%12d | ${s64.steps.toDouble / s16.steps}%8.3f"
            )
    ...
    test("suffix cost: suffix16 vs suffix64") {
        ...
        val header =
            f"${"Cursor"}%-12s | ${"s16 mem"}%8s | ${"s16 cpu"}%12s | ${"s64 mem"}%8s | ${"s64 cpu"}%12s | ${"s64/s16"}%8s"
        info(header)
        info("-" * header.length)
        ...
        for i <- cursors16.indices do
            ...
            info(
              f"${c16}/${c64}%-12s | ${s16.memory}%8d | ${s16.steps}%12d | ${s64.memory}%8d | ${s64.steps}%12d | ${s64.steps.toDouble / s16.steps}%8.3f"
            )
