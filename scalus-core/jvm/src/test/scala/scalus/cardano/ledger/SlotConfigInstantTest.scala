...
/** The `java.time.Instant` conversions on [[SlotConfig]].
  *
  * JVM-only because the Scala.js `SlotConfig` deliberately has no `Instant` members: every public
  * member of a `@JSExportTopLevel` `js.Object` is a linker export root, and one mentioning
  * `Instant` pulls `java.time` and its ~800 KB timezone database into `scalus.js` for an API
  * JavaScript callers cannot use. JS callers work in POSIX milliseconds through `slotToTime` and
  * `timeToSlot`, which `SlotConfigTest` covers on every platform. See
  * `docs/internal/JS_BUNDLE_SIZE.md`.
  */
@nowarn("msg=(toLong is redundant|long2double)")
class SlotConfigInstantTest extends AnyFunSuite with ScalaCheckPropertyChecks {
