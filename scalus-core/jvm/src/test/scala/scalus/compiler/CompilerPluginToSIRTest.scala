...
class CompilerPluginToSIRTest extends AnyFunSuite with ScalaCheckPropertyChecks:
    ...
    test("compile lambda with args with type parameters") {
        ...
        val a = TypeVar("A", Some(1), SIRType.TypeVarKind.ListAffected)
    ...
    test("compile ToData") {
        ...
        val expected = {
            ...
            then {
                val a1Tp =
                    SIRType.TypeVar("A", Some(1), SIRType.TypeVarKind.ListAffected)
                val a2Tp =
                    SIRType.TypeVar("A", Some(2), SIRType.TypeVarKind.ListAffected)
                val a3Tp =
                    SIRType.TypeVar("A", Some(3), SIRType.TypeVarKind.ListAffected)
                Let(
                  immutable.List(
