...
import scalus.{toUplc, toUplcOptimized, uplc}
...
class CompilerPluginToSIRTest extends AnyFunSuite with ScalaCheckPropertyChecks:
    ...
    test("compile lambda with args with type parameters") {
        ...
        val a = TypeVar("A", Some(1), SIRType.TypeVarKind.Fixed)
    ...
    test("compile ToData") {
        ...
        val expected = {
            ...
            then {
                val a1Tp =
                    SIRType.TypeVar("A", Some(1), SIRType.TypeVarKind.Fixed)
                val a2Tp =
                    SIRType.TypeVar("A", Some(2), SIRType.TypeVarKind.Fixed)
                val a3Tp =
                    SIRType.TypeVar("A", Some(3), SIRType.TypeVarKind.Fixed)
                Let(
                  immutable.List(
